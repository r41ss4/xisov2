# About 
The project is a partial design for fintech (XISO) application database using MySQL. The database is design for users to be able to charge money in the fintechs account, withdraw and deposit in merchants. It aims to support basic functionalities. 
...
If seeds and other parts of the analytics projects are desired to be execute, such as seed_heavy_users.sql, this can be done through the terminal and connecting to mysql within the container. This process can be done with the seeds and the files in db/05_analysis. The following commands can help to do so:
```bash
# Run db/04_seeds/seed_heavy_users.sql in container
docker exec -i xiso-container mysql -u root -p root_password xiso_staging < db/04_seeds/seed_heavy_users.sql
...
docker exec -i xiso-container mysql -u root -p  root_password xiso_staging < db/05_analysis/transaction_summary.sql

# Run db/05_analysis/user_balance.sql in container
docker exec -i xiso-container mysql -u root -p  root_password xiso_staging < db/05_analysis/user_balance.sql
...
The rest of the commands, such as the way to run db/04_seeds/seed_heavy_users.sql is similar, skipping the `mysql -u root -p` command.

```bash
docker exec -i xiso-container mysql -u root -p  root_password xiso_staging < db/04_seeds/seed_heavy_users.sql
```

Moreover, for the python/ analysis to work if the db is dockerized, the python/main.py file must be modified to adapt. Therefore, open python/db.py and spot the following snippet:
...
```bash
connection = create_connection("root", "root", "xiso_staging")
```